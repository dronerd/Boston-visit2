    ...
    <div className="min-h-screen bg-gradient-to-b from-[#071536] via-[#0b3b73] to-[#04162a] text-zinc-100">
      <main className="mx-auto max-w-6xl px-6 py-16">
        ...
        <div className="space-y-12">
          {schedule.map((day) => (
            <section key={day.date} className="rounded-2xl bg-white/4 ring-1 ring-white/8 overflow-hidden">
              {/* Day Header */}
                  ...
                  <div key={session.id} className="px-8 py-6 hover:bg-white/3 transition-colors">
                    <div className="flex gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        {session.guests && session.guests.length > 0 && (
                          <div className="mt-4 space-y-3">
                            {session.guests.map((guest, idx) => (
                              <div
                                key={idx}
                                className="rounded-lg bg-white/5 p-4 ring-1 ring-white/6"
                              >
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            {session.references.map((ref, idx) => (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-block px-3 py-1 rounded-full bg-blue-500/20 text-xs text-blue-300 hover:bg-blue-500/30 transition-colors"
                              >
                                {ref.label} →
                              </a>
                            ))}
                        ...
                        {/* Notes */}
                        {session.notes && (
                          <p className="mt-3 text-xs text-zinc-400 italic">ℹ️ {session.notes}</p>
                        )}
                      </div>
                    </div>
                  </div>
